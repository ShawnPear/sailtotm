...
public interface OrderMapper {
    ...
    @Select("SELECT * from SAILTOTM.Orders where user_id = #{userId} and order_id = #{orderId}")
    Order getOrder(String userId, String orderId);

    @Update("update SAILTOTM.Orders set status_id = 0 where order_id = #{orderId}")
    ...
    @Update("update SAILTOTM.Orders set status_id = #{status} , updated_date = #{updatedDate} where order_id = #{orderId}")
    Boolean setOrderStatus(String orderId, Integer status, LocalDateTime updatedDate);

    @Insert("insert into SAILTOTM.OrderStatusChangeHIstory (created_date, order_id, stuff_id, old_status, new_status) VALUES (#{createdDate},#{orderId},#{stuffId},#{oldStatus},#{newStatus})")
    Boolean addStatusChangeHistory(OrderStatusChange data);

    @Update("update SAILTOTM.Orders set pickup_code = #{pickupCode} where order_id = #{orderId} ")
