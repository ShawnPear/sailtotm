...
public interface SearchItemMapper {
    ...
    @Insert("insert into SAILTOTM.SearchItems (user_id, search, searched_times, created_date,updated_date) VALUES " +
            "(#{userId},#{search},#{searchedTimes},#{createdDate},#{updatedDate})")
    Boolean add(SearchItem item);

    @Update("update SAILTOTM.SearchItems " +
    ...
    @Select("select * from SAILTOTM.SearchItems where user_id = #{userId} and search = #{search}")
    SearchItem selectByQandUserId(String search, String userId);

    @Select("select * from SAILTOTM.SearchItems where user_id = #{userId} order by updated_date desc")
